...
**N = 1 reviewer(s), 27 scored decisions.**
...
| --- | --- | --- | --- | --- | --- | --- |
| `batch_section` | 7 | 63.4s | 48.8s–97.7s | — (single verdict) | — | — |
| `batch_whole` | 7 | 45.9s | 34.9s–56.6s | — (single verdict) | — | — |
| `sequential_section` | 6 | 13.8s | 5.1s–15.5s | — (single verdict) | — | — |
| `sequential_whole` | 7 | 14.4s | 11.9s–25.3s | — (single verdict) | — | — |

## Data quality warnings

- **Every decision was `approve`.** The reviewer never used the other verdict, so nothing here measures discrimination. Per-condition accuracy is suppressed in the table above rather than printed: the figure would be the corpus base rate wearing an accuracy label. The error breakdown below is still informative — it says which seeded errors a blanket approval lets through — but it is a property of the corpus, not of the reviewer.

Treat the per-condition figures above as provisional until a run without these warnings exists.

### Per-condition quality

| Condition | <2s | No engagement signal | Verdicts used |
| --- | --- | --- | --- |
| `batch_section` | 0/7 | 0 | approve |
| `batch_whole` | 0/7 | 0 | approve |
| `sequential_section` | 1/6 | 0 | approve |
| `sequential_whole` | 0/7 | 0 | approve |

## Errors that slipped through

- `batch_section` — wrongly approved: wrong_number: 1
- `batch_whole` — wrongly approved: meaning_flip: 1, wrong_number: 1
- `sequential_section` — wrongly approved: figure_carryover: 1, unrequested_substantive_change: 1
- `sequential_whole` — wrongly approved: obligation_gutted: 1, wrong_number: 1
